"""
Order pricing: buyer service fee + seller withholding.
...
def _buyer_fee_rate() -> Decimal:
    return get_fee_rates().base_buyer_fee_rate


def _seller_fee_rate() -> Decimal:
    return get_fee_rates().base_seller_fee_rate


def _affiliate_buyer_discount_rate() -> Decimal:
    return get_fee_rates().buyer_coupon_discount_rate


def _affiliate_commission_rate() -> Decimal:
    return get_fee_rates().affiliate_commission_rate


def _affiliate_platform_net_rate() -> Decimal:
    return get_fee_rates().affiliate_platform_net_rate
...
def affiliate_checkout_amounts(
    ...
    *,
    ...
    """
    Coupon economics on listing base (defaults from GlobalFeeSettings affiliate path):

      buyer_fee = base * (full_fee_rate - buyer_discount_rate)
      buyer_discount / affiliate_commission / platform_net_fee from rates
      total = base + buyer_fee

    Callers that pass explicit rates (e.g. platform coupon with affiliate=0) override defaults.
    """
    b = decimal_money(base)
    if b <= 0:
        zero = Decimal('0.00')
    ...
    fees = get_fee_rates()
    disc = Decimal(
        str(buyer_discount_rate if buyer_discount_rate is not None else fees.buyer_coupon_discount_rate)
    )
    aff = Decimal(str(affiliate_rate if affiliate_rate is not None else fees.affiliate_commission_rate))
    if platform_rate is not None:
        plat = Decimal(str(platform_rate))
    else:
        plat = fees.affiliate_platform_net_rate

    full = fees.base_buyer_fee_rate
