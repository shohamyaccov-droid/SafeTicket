...
<div style="max-width:560px;margin:0 auto;padding:24px;background:#ffffff;border-radius:12px;border:1px solid #e2e8f0;">
  <h1 style="font-size:20px;color:#0f766e;margin:0 0 12px;">TradeTix — תודה שרכשתם אצלנו</h1>
  ...
  {guest_note}
  <p style="margin:24px 0 0;font-size:13px;color:#94a3b8;">בברכה,<br/>צוות TradeTix</p>
</div></body></html>'''

    text_lines = [
        'TradeTix — קבלה והודעת תודה',
        '',
        f'אירוע: {raw_event}',
        f'כמות: {qty}',
    ...
    text_lines.extend(['', '— צוות TradeTix'])
    text_body = '\n'.join(text_lines)

    subject = f'TradeTix — קבלה עבור {raw_event} (הזמנה #{order_id})'
...
def send_otp_email(user, otp):
    ...
    """
    subject = 'TradeTix - Verify your email'
...
If you did not register for TradeTix, please ignore this email.
