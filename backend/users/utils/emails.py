...
<div style="max-width:560px;margin:0 auto;padding:24px;background:#ffffff;border-radius:12px;border:1px solid #e2e8f0;">
  <h1 style="font-size:20px;color:#0f766e;margin:0 0 12px;">TixTrade — תודה שרכשתם אצלנו</h1>
  ...
  {guest_note}
  <p style="margin:24px 0 0;font-size:13px;color:#94a3b8;">בברכה,<br/>צוות TixTrade</p>
</div></body></html>'''

    text_lines = [
        'TixTrade — קבלה והודעת תודה',
        '',
        f'אירוע: {raw_event}',
        f'כמות: {qty}',
    ...
    text_lines.extend(['', '— צוות TixTrade'])
    text_body = '\n'.join(text_lines)

    subject = f'TixTrade — קבלה עבור {raw_event} (הזמנה #{order_id})'
...
def send_otp_email(user, otp):
    ...
    """
    subject = 'TixTrade - Verify your email'
...
If you did not register for TixTrade, please ignore this email.
...
def send_offer_notification(recipient_email, offer_details):
    ...
    subject = f'TixTrade - New offer on {event_name}'
    body = f'''Hello,

{buyer_username} has made an offer of ₪{amount} on your listing for {event_name}.

Log in to TixTrade to accept or reject the offer.
