// @ts-check
/**
 * Full iPhone-style journey: two distinct UI-registered users — seller lists, buyer bargains,
 * seller accepts, buyer checks out + PDF. Verifies Hebrew toast copy and pricing math.
 */
...
async function registerUi(page, webBase, { first, last, email, password }) {
...
test.describe('iPhone full lifecycle (two UI users)', () => {
  test('seller registers + list; buyer registers + offer; accept; checkout; PDF + math', async ({
    page,
  }) => {
    ...
    const ts = Date.now();
    const sellerEmail = `e2e_seller_${ts}@e2e.local`;
    const buyerEmail = `e2e_buyer_${ts}@e2e.local`;
    const pass = 'E2EFullJourney2026!b';
    const expectTotal = expectedBuyerTotalFromBase(OFFER_BASE);

    const pdfPath = path.join(os.tmpdir(), `safeticket-full-${ts}.pdf`);
    fs.writeFileSync(pdfPath, MINIMAL_PDF, 'utf8');

    await registerUi(page, webBase, {
      first: 'Seller',
      last: `S${ts}`,
      email: sellerEmail,
      password: pass,
    ...
    await page.locator('.become-seller-modal input[type="tel"]').fill('050-9998877');
    await page.locator('.become-seller-modal textarea').fill('paypal: fulljourney@example.com');
    ...
    await expect(page.getByText('התחברת בהצלחה', { exact: false })).toBeVisible({
      timeout: 30_000,
    });
    ...
    await page.locator('#cardholderName').fill('E2E Full Journey');
