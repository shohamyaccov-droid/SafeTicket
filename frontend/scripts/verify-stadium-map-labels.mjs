...
const checks = [
  ...
  ['grandstand 2-3 center', /'2-3':\s*\{\s*x:\s*545,\s*y:\s*745\s*\}/],
  ['grandstand 1 right', /'1':\s*\{\s*x:\s*841,\s*y:\s*720\s*\}/],
  ['viagogo bottom row comment', /4,\s*3,\s*2-3,\s*2,\s*1/],
  ['render uses grandstandCoords', /grandstandCoords[\s\S]*?\? grandstandCoords\.x/],
  ['labels use dy em', /dy="\.3em"/],
  ['no dominantBaseline on labels', () => !/dominantBaseline/.test(mapSrc)],
